...
class RestAPISearcher(SearcherBase):
    ...
    def invoke(self, question_value: str) -> SearchResp:
        ...
        try:
            if self.method == 'GET':
                resp = self.session.get(self.url, params={self.req_field: self.question_value, **self.ext_params})
            elif self.method == 'POST':
                resp = self.session.post(self.url, data={self.req_field: self.question_value, **self.ext_params})
            else:
                raise TypeError
...
class EnncySearcher(RestAPISearcher):
    ...
    def parse(self, json_content: dict) -> SearchResp:
        ...
        if ''.join(jsonpath.compile('$.data.answer').parse(json_content)) in ('配置为空或者配置错误，请自行检查或者联系作者查看。', '题库配置的“凭证”被刷新，不要刷新你的凭证！只有当你的题库被别人盗用时才能进行刷新操作，否则会导致题库配置失效，请您前往 https://tk.enncy.cn/ 登录后到个人中心复制题库配置，并重新在脚本设置中粘贴题库配置。'):
...
class SqliteSearcher(SearcherBase):
    ...
    def __init__(self, file_path: Union[Path, str], req_field: str='question', rsp_field: str='answer', table: str='question') -> None:
...
__all__ = ['SearcherBase', 'RestAPISearcher', 'JsonFileSearcher', 'SqliteSearcher', 'EnncySearcher']