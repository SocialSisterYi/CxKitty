...
class SearchRespShowComp:
    ...
    def __rich_console__(self, console: Console, options: ConsoleOptions) -> RenderResult:
        ...
        yield Group(
            Text("q: ", end=""),
            Text(self.question.value, style="cyan")
        )
            ...
            yield Group(
                ...
                Styled(
                    Group(
                        ...
                        Text("Ok" if result.code == 0 else f"Err {result.code}:{result.message}", end=""),
    ...
    def execute(self) -> None:
        ...
        for index, question in self.exam_dto:
            ...
            if status == False:
                ...
                self.mistakes.append((
                    question,
                    "/".join(str(result.answer) for result in results)
                ))
            else:
            ...
            try:
                ...
                self.logger.warning(
                    f"题目提交失败 -> {e.__class__.__name__} {e.__str__()} "
                    f"[{self.exam_dto}]"
                )
                msg_console.update(Panel(
        ...
        if self.incompleted_cnt == 0:
            ...
            try:
                ...
                self.logger.warning(
                    f"试题交卷失败 -> {e.__class__.__name__} {e.__str__()} "
                    f"[{self.exam_dto}]"
                )
                msg_console.update(Panel(
                    f"{e.__class__.__name__} {e.__str__()}",
        ...
        # 存在错误
            ...
            if self.enable_fallback_save is True:
                try:
                    ...
                    self.logger.warning(
                        f"试题保存失败 -> {e.__class__.__name__} {e.__str__()} "
                        f"[{self.exam_dto}]"
                    )
                    msg_console.update(Panel(
                        f"{e.__class__.__name__} {e.__str__()}",
